...
def provev10(loi):
	...
	tthree = abs((loi[2] + nxt_len) - loi[3]) <= 1
	tfour = (loi[3] - loi[2]) <= 14
	if(tone and ttwo and tthree and tfour):
		return(True, [loi[0] - barcode1_len, loi[1] - barcode2_len, loi[2] - barcode3_len, loi[3]])
	else:
...
def resolveIndex(c1_hit, c2_hit, nxt_hit, me_hit):
	...
	if(len(c1_hit) < 1 or len(c2_hit) < 1 or len(nxt_hit) < 1 or len(me_hit) < 1):
		return(False, [len(c1_hit) > 0, len(c2_hit) > 0, len(nxt_hit) > 0, len(me_hit) > 0, 1])
	else:
		...
		fnout = list([provev10(x) for x in possibilities])
		...
		if any(verified):
			...
			return(False, [len(c1_hit) > 0, len(c2_hit) > 0, len(nxt_hit) > 0, len(me_hit) > 0, 0])
...
while 1:
	...
	if(idx[0]):
		...
		if(bc1 != "N"*barcode1_len and bc2 != "N"*barcode2_len and bc3 != "N"*barcode3_len):
			# Add barcode to sequence header
			seqhead1 = seqhead1.replace(" ", "") + "_CB:" + barcode
			seqhead2 = seqhead2.replace(" ", "") + "_CB:" + barcode
		
			# Cut out the barcode 
			seq1 = seq1[(idx[1][3] + me_len):len(seq1)]
			qual1 = qual1[(idx[1][3] + me_len):len(qual1)]
